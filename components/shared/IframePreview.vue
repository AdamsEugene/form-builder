<template>
    ...
    <div
        v-if="isLoading"
        class="absolute inset-0 flex items-center justify-center bg-white/80 dark:bg-gray-800/80 z-10 rounded-2xl"
    >
        <div
            class="w-8 h-8 border-4 border-gray-200 dark:border-gray-700 border-t-primary-500 rounded-full animate-spin"
        />
    </div>
    <!-- Iframe with initial loading state -->
    <iframe
        ref="frameRef"
...
const mountComponent = () => {
    if (!frameRef.value || !frameRef.value.contentWindow) return;

    const iframeDocument = frameRef.value.contentWindow.document;
        ...
        if (mountPoint) {
            ...
            const resizeObserver = new ResizeObserver((entries) => {
                for (const entry of entries) {
                    if (entry.target === iframeDocument.body) {
                        frameHeight.value = entry.target.scrollHeight;
                    }
                }
            });
